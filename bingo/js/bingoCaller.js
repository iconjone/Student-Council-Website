...
var Bingo = function(bingoBoardElement, speechInstance) {
     ...
     * @returns {*}
     */
    function add15Balls(allBingoNumbers, currentBingoBall, letterBlock, letter,tier) {
      var tier = tier;
        var totalBingoBalls = currentBingoBall + (tier/5);
        for (currentBingoBall; currentBingoBall < totalBingoBalls; currentBingoBall++) {
            var newBingoBall = helper.createDomElement('div', 'ball valign halign ' + letter + currentBingoBall);
...
window.onkeyup = function(e) {
   if(e.keyCode == 38 || e.keyCode == 39 || e.keyCode == 34){
   console.log("test");
   callBingoBall();
 }
 }
